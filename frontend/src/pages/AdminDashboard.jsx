    ...
    const load = async () => {
        ...
        try {
            const [a, b, c] = await Promise.all([
                fetch('/api/societies/admin/stats', { headers, credentials: 'include' }),
                fetch('/api/societies/admin/members', { headers, credentials: 'include' }),
                fetch('/api/societies/admin/requests', { headers, credentials: 'include' }),
            ]);
            setStats(await a.json());
            setMembers(await b.json());
            setRequests(await c.json());
        } finally {
            setLoading(false);
        }
    };
...
function PlatformAdminView({ token }) {
    ...
    return (
        <div className="admin-dashboard animate-in">
            ...
            {tab === 'subscriptions' && <section className="admin-panel"><div className="panel-head"><div><h2>Subscription management</h2><p>Control plan and billing state for every society.</p></div></div><div className="table-like">{societies.map((s) => <div key={s.id} className="table-row"><div className="table-main"><strong>{s.name}</strong><span>{s.city}</span><span>Ends {fmt(s.subscriptionEndsAt)}</span></div><div className="table-meta"><span className="status-badge" style={{ backgroundColor: `${approvalColors[s.approvalStatus]}1a`, color: approvalColors[s.approvalStatus] }}>{s.approvalStatus}</span><span className="status-badge" style={{ backgroundColor: `${subColors[s.subscriptionStatus]}1a`, color: subColors[s.subscriptionStatus] }}>{s.subscriptionStatus}</span></div><div className="table-actions table-actions-wrap"><select className="table-select" value={s.subscriptionPlan} onChange={(e) => patch(`/api/societies/admin/platform/societies/${s.id}/subscription`, { subscriptionPlan: e.target.value, subscriptionStatus: s.subscriptionStatus, subscriptionEndsAt: s.subscriptionEndsAt })}><option value="FREE">FREE</option><option value="PRO">PRO</option><option value="ENTERPRISE">ENTERPRISE</option></select><select className="table-select" value={s.subscriptionStatus} onChange={(e) => patch(`/api/societies/admin/platform/societies/${s.id}/subscription`, { subscriptionPlan: s.subscriptionPlan, subscriptionStatus: e.target.value, subscriptionEndsAt: s.subscriptionEndsAt })}><option value="TRIALING">TRIALING</option><option value="ACTIVE">ACTIVE</option><option value="PAST_DUE">PAST_DUE</option><option value="CANCELED">CANCELED</option></select></div></div>)}</div></section>}
