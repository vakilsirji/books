...
router.post('/', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        await prisma.user.update({
            ...
            data: { societyId: society.id, role: 'SOCIETY_ADMIN' }
        });
        ...
        res.status(201).json({
            society,
            user: { ...req.user, societyId: society.id, role: 'SOCIETY_ADMIN', society: { name: society.name } },
            token
        });
    } catch (error) {
...
router.post('/join', authMiddleware, async (req, res) => {
    ...
    try {
        const society = await prisma.society.findUnique({ where: { accessCode } });
        if (!society) return res.status(404).json({ error: 'Invalid access code' });

        const user = await prisma.user.update({
            where: { id: req.user.id },
            data: { societyId: society.id, status: 'PENDING' },
...
router.get('/admin/stats', authMiddleware, adminOnly, async (req, res) => {
    ...
    try {
        const [memberCount, bookCount, requestCount, pendingCount] = await Promise.all([
            prisma.user.count({ where: { societyId, status: 'ACTIVE' } }),
            prisma.book.count({ where: { societyId } }),
...
router.get('/admin/platform/overview', authMiddleware, platformOnly, async (req, res) => {
    try {
        const [totals, societies] = await Promise.all([
            Promise.all([
                prisma.society.count(),
                prisma.society.count({ where: { approvalStatus: 'PENDING' } }),
                prisma.society.count({ where: { approvalStatus: 'APPROVED' } }),
                prisma.society.count({ where: { subscriptionStatus: 'ACTIVE' } }),
                prisma.user.count(),
                prisma.book.count()
            ]),
